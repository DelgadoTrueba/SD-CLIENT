...
public class BoardView implements java.util.Observer{
	...
	public void update(Observable obs, Object obj) {
		...
		if(obs instanceof ObservableBoarModel && ActionsBoardModel.setHiddenCardJ1.equals(notification.action)) {
			
			row = notification.row;
			col = notification.col;
			
			
			this.btnBoard[row][col].setBackground(Color.CYAN);
		...
		if(obs instanceof ObservableBoarModel && ActionsBoardModel.setHiddenCardJ2.equals(notification.action)) {
			
			row = notification.row;
			col = notification.col;
			
			
			this.btnBoard[row][col].setBackground(Color.YELLOW);
	...
	public void playerTie() {
		JOptionPane.showMessageDialog(null, "You TIE :( ", "RESULT", JOptionPane.INFORMATION_MESSAGE);
		System.exit(0);
	}
